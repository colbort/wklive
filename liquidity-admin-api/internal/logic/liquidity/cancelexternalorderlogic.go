// Code scaffolded by goctl. Safe to edit.
// goctl 1.9.2

package liquidity

import (
	"context"

	"wklive/liquidity-admin-api/internal/svc"
	"wklive/liquidity-admin-api/internal/types"

	"github.com/zeromicro/go-zero/core/logx"
)

type CancelExternalOrderLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewCancelExternalOrderLogic(ctx context.Context, svcCtx *svc.ServiceContext) *CancelExternalOrderLogic {
	return &CancelExternalOrderLogic{
		Logger: logx.WithContext(ctx),
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

func (l *CancelExternalOrderLogic) CancelExternalOrder(req *types.CancelExternalOrderReq) (resp *types.RespBase, err error) {
	return cancelExternalOrder(l.ctx, l.svcCtx, req)
}
