...
COMPOSE_FILE="$DEPLOY_ROOT/common/compose.base.yml"
DOCKERFILE="$DEPLOY_ROOT/common/docker/Dockerfile.beanstalkd"
RESTART_SMOKE="$SCRIPT_DIR/restart-smoke.sh"
RESILIENCE_SMOKE="$SCRIPT_DIR/resilience-smoke.sh"
failures=0
...
require_contains "$COMPOSE_FILE" \
  'dockerfile: deploy/common/docker/Dockerfile[.]beanstalkd' \
...
for service_name in beanstalk-primary beanstalk-secondary; do
  container_id=$(dev_compose ps -q "$service_name" 2>/dev/null)
