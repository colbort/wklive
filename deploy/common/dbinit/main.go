...
type config struct {
	...
	Profile                string
	ProfileSQL             string
	PreprodTenantID        int64
	PreprodTenantCode      string
	PreprodTenantName      string
}
...
func main() {
	...
	if cfg.DataOnly {
		...
		must(applyDeploymentProfile(ctx, db, cfg))
...
func loadConfig() config {
	...
	profile := strings.ToLower(getenv("DB_INIT_PROFILE", ""))
	cfg := config{
		DSN:                    loadMySqlDSN(),
		Workspace:              workspace,
		...
		Profile:                profile,
		ProfileSQL:             getenv("DB_INIT_PROFILE_SQL", filepath.Join(workspace, "profiles", "preprod", "bootstrap.sql")),
	}
	...
	if profile != "" && profile != "preprod" {
		log.Fatalf("DB_INIT_PROFILE must be empty or preprod, found %q", profile)
	}
	if profile == "preprod" {
		tenantID, err := parsePositiveInt64("PREPROD_TENANT_ID", getenv("PREPROD_TENANT_ID", "1"))
		must(err)
		cfg.PreprodTenantID = tenantID
		cfg.PreprodTenantCode = getenv("PREPROD_TENANT_CODE", "preprod")
		cfg.PreprodTenantName = getenv("PREPROD_TENANT_NAME", "预发布租户")
		validateIdentifier("PREPROD_TENANT_CODE", cfg.PreprodTenantCode)
		if strings.TrimSpace(cfg.PreprodTenantName) == "" {
			log.Fatal("PREPROD_TENANT_NAME must not be empty")
		}
	}
...
func validateIdentifier(name, value string) {
	if value == "" {
	...
	for _, char := range value {
		...
		log.Fatalf("%s may only contain letters, numbers, dot, underscore and dash", name)
	}
}
...
func applyDeploymentProfile(ctx context.Context, db *sql.DB, cfg config) error {
	if cfg.Profile == "" {
		return nil
	}
	if cfg.Profile != "preprod" {
		return fmt.Errorf("unsupported database initialization profile: %s", cfg.Profile)
	}

	profilePath := cfg.ProfileSQL
	connection, err := db.Conn(ctx)
	if err != nil {
		return fmt.Errorf("acquire pre-production seed connection: %w", err)
	}
	defer connection.Close()

	var idConflict, codeConflict int64
	err = connection.QueryRowContext(ctx, `
SELECT
  COALESCE(SUM(id=? AND tenant_code<>?),0),
  COALESCE(SUM(tenant_code=? AND id<>?),0)
FROM sys_tenant
WHERE id=? OR tenant_code=?`,
		cfg.PreprodTenantID, cfg.PreprodTenantCode,
		cfg.PreprodTenantCode, cfg.PreprodTenantID,
		cfg.PreprodTenantID, cfg.PreprodTenantCode,
	).Scan(&idConflict, &codeConflict)
	if err != nil {
		return fmt.Errorf("validate pre-production tenant identity: %w", err)
	}
	if idConflict > 0 || codeConflict > 0 {
		return fmt.Errorf(
			"pre-production tenant id/code conflicts with existing data: id=%d code=%s",
			cfg.PreprodTenantID, cfg.PreprodTenantCode,
		)
	}

	if _, err = connection.ExecContext(ctx, `
SET @preprod_tenant_id=?, @preprod_tenant_code=?, @preprod_tenant_name=?`,
		cfg.PreprodTenantID, cfg.PreprodTenantCode, cfg.PreprodTenantName,
	); err != nil {
		return fmt.Errorf("bind pre-production seed variables: %w", err)
	}
	if err = execSQLFileWithExecutor(ctx, connection, profilePath); err != nil {
		return fmt.Errorf("apply pre-production seed %s: %w", profilePath, err)
	}
	log.Printf(
		"pre-production baseline data merged: tenant_id=%d tenant_code=%s symbols=BTCUSDT,ETHUSDT",
		cfg.PreprodTenantID, cfg.PreprodTenantCode,
	)
	return nil
}
...
func execSQLFile(ctx context.Context, db *sql.DB, path string) error {
	return execSQLFileWithExecutor(ctx, db, path)
}

func execSQLFileWithExecutor(ctx context.Context, executor sqlExecutor, path string) error {
	data, err := os.ReadFile(path)
	if err != nil {
		return err
	}
