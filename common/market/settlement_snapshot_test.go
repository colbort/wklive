...
func TestAuthoritativeQuoteArchivePreservesDecimalAndHistoricalTime(t *testing.T) {
	...
	for _, quote := range []*QuotePayload{
		{LastPrice: 1.2345678901234567, LastPriceText: "1.234567890123456789", Ts: 1000, Authority: "market-ws"},
		{LastPrice: 2, LastPriceText: "2.000000000000000001", Ts: 2000, Authority: "market-ws"},
	} {
		if _, err := cache.PublishAuthoritativeQuote(ctx, msg, quote); err != nil {
			t.Fatal(err)
		}
	}
	got, err := cache.FindAuthoritativeQuoteAt(ctx, msg, "market-ws", 1500, time.Second)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAuthoritativeQuoteHandlerFailureDoesNotPublishRealtimeCache(t *testing.T) {
	...
	msg := ClientMessage{Topic: TopicQuote, CategoryCode: "crypto", Market: "BA", Symbol: "BTCUSDT"}
	err := cache.Set(context.Background(), msg, &QuotePayload{LastPrice: 1, LastPriceText: "1", Ts: 1000, Authority: "market-ws"})
	if err == nil {
...
func TestAuthoritativeSnapshotCacheUsesBoundedV3Layout(t *testing.T) {
	...
	ctx := context.Background()
	msg := ClientMessage{Topic: TopicQuote, CategoryCode: "crypto", Market: "BA", Symbol: "BTCUSDT"}
	first := &SettlementSnapshot{SnapshotID: "first", Authority: "market-ws", Kind: "FINAL_QUOTE", CategoryCode: "crypto", Market: "BA", Symbol: "BTCUSDT", Price: "100", SourceTimestamp: 1000, SnapshotTimestamp: 1001, Revision: 1000, Confirmed: true}
	second := &SettlementSnapshot{SnapshotID: "second", Authority: "market-ws", Kind: "FINAL_QUOTE", CategoryCode: "crypto", Market: "BA", Symbol: "BTCUSDT", Price: "101", SourceTimestamp: 61_001, SnapshotTimestamp: 61_002, Revision: 61_001, Confirmed: true}
	...
	if count := client.ZCard(ctx, authoritativeHotSnapshotKey(msg, "market-ws", "FINAL_QUOTE")).Val(); count != 1 {
		t.Fatalf("hot snapshot count=%d want=1", count)
	}
	latest, err := cache.FindLatestAuthoritativeSnapshot(ctx, msg, "market-ws", "FINAL_QUOTE")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCleanupLegacyAuthoritativeCachePreservesRevocations(t *testing.T) {
	...
	legacyIndex := "market:authoritative:v2:index:market-ws:FINAL_QUOTE:crypto:BA:BTCUSDT"
...
func TestAuthoritativeQuoteRejectsDatabasePrecisionOverflow(t *testing.T) {
	...
	for _, price := range []string{"1.1234567890123456789012345678901", "123456789012345678901234567890123456"} {
		if _, err := BuildAuthoritativeQuoteSnapshot(msg, &QuotePayload{LastPriceText: price, Ts: 1, Authority: "market-ws"}); err == nil {
