...
import {
  apiCreateCryptoRechargeOrder,
  apiGetMyCryptoRechargeAddress,
} from "@/api/payment";
...
const selectedCoin = computed(
  () => selectedConfig.value?.coin || routeCoin.value,
);
...
function getChainLabel(config: AssetCoinConfig) {
  if (config.chainCode)
    return chainLabels[config.chainCode] || String(config.chainCode);
...
<template>
  <AssetFlowLayout
    title="充值"
    :right-text="step === 'select' ? '资金记录' : undefined"
    narrow
  >
