...
const fromConfig = computed(() => coinConfigs.value.find((config) => config.coin === fromCoin.value))
const toConfig = computed(() => coinConfigs.value.find((config) => config.coin === toCoin.value))
const pickerWalletType = computed(() => (pickerTarget.value === 'to' ? toWalletType.value : fromWalletType.value))
...
const availableAmount = computed(() => {
  ...
  return assets.value.find((asset) => asset.walletType === fromWalletType.value && asset.coin === fromCoin.value)?.availableAmount || '0'
})
...
async function loadPageData() {
  try {
    ...
    coinConfigs.value = configResponses.flatMap((resp) => (isSuccessCode(resp.code) ? resp.data || [] : []))
  } catch (error) {
    ...
    <section class="transfer-card">
      <label class="transfer-field">
        <span class="transfer-field__head">
          <strong>转出</strong>
          <small v-if="fromCoin">可用 <b>{{ availableAmount }}</b> {{ fromCoin }}</small>
        </span>
