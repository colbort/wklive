...
func (w *SyncKlinesWorker) Run(taskNo string, apiUrl string, token string) {
	w.runTask(taskNo, "历史回补中", func() error { return w.doSync(apiUrl, token) })
}
...
func (w *SyncKlinesWorker) reconcileBatch(apiURL, token string, group *reconcileGroup, products []*models.TItickProduct) error {
	...
	data, err := w.getBatchKlines(w.ctx, apiURL, token, group.category, group.market, group.exchange, codes, reconcileKType, 0, reconcileWindowBars)
	if err != nil {
...
func (w *SyncKlinesWorker) getBatchKlines(ctx context.Context, apiURL, token, category, market, exchange string,
	codes []string, kType int32, et int64, limit int) (map[string][]ItickKlineItem, error) {
...
func (w *SyncKlinesWorker) SyncCatchup(job KlineJob, interval string, contiguousTs int64, now int64) (SyncResult, error) {
	const limit = 100
	...
	if contiguousTs >= to {
		return SyncResult{LatestTs: contiguousTs, ReachedBase: true}, nil
	}
...
func (w *SyncKlinesWorker) syncRecentCheck(job KlineJob, interval string, now int64) (SyncResult, error) {
...
func (w *SyncKlinesWorker) SyncHistory(job KlineJob, interval string, oldestTs int64, now int64) (SyncResult, error) {
	const limit = 300
...
func (w *SyncKlinesWorker) syncBackwardRange(job KlineJob, interval string, et int64, stopAtTs int64, maxAcceptTs int64, limit int, maxPages int) (SyncResult, error) {
...
func (w *SyncKlinesWorker) getKlineFromItick(
	...
	data, err := w.getBatchKlines(ctx, apiURL, token, category, market, "", []string{symbol}, job.KType, et, limit)
	if err != nil {
		return nil, err
	}
	items, ok := data[symbol]
	if !ok {
		for code, candidate := range data {
			if strings.EqualFold(code, symbol) {
				items, ok = candidate, true
				break
			}
		}
	}
	if !ok {
		return nil, fmt.Errorf("batch kline response missing symbol=%s", symbol)
	}
	return &ItickKlineResponse{Data: items}, nil
}
