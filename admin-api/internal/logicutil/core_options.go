...
func StakingOptions() []types.OptionsGroup {
	return []types.OptionsGroup{
		...
		Group("stakingOrderStatus", "质押订单状态",
			Option(0, "STAKING_ORDER_STATUS_UNKNOWN"),
			Option(1, "STAKING_ORDER_STATUS_STAKING"),
			Option(2, "STAKING_ORDER_STATUS_EXPIRED"),
			Option(3, "STAKING_ORDER_STATUS_REDEEMED"),
			Option(4, "STAKING_ORDER_STATUS_EARLY_REDEEMED"),
			Option(5, "STAKING_ORDER_STATUS_CANCELLED"),
			Option(6, "STAKING_ORDER_STATUS_PENDING"),
		),
		EnumGroup("stakingRedeemType", "质押赎回类型", staking.RedeemType_REDEEM_TYPE_UNKNOWN.Descriptor()),
		EnumGroup("stakingRewardType", "质押收益类型", staking.RewardType_REWARD_TYPE_UNKNOWN.Descriptor()),
		EnumGroup("stakingRewardStatus", "质押收益状态", staking.RewardStatus_REWARD_STATUS_UNKNOWN.Descriptor()),
		EnumGroup("stakingRedeemStatus", "质押赎回状态", staking.RedeemStatus_REDEEM_STATUS_UNKNOWN.Descriptor()),
		EnumGroup("stakingSourceType", "质押来源", staking.SourceType_SOURCE_TYPE_UNKNOWN.Descriptor()),
		Group("stakingOperationType", "质押操作类型",
			Option(1, "STAKING_OPERATION_SUBSCRIBE"),
			Option(2, "STAKING_OPERATION_DAILY_REWARD"),
			Option(3, "STAKING_OPERATION_MATURITY_REWARD"),
			Option(4, "STAKING_OPERATION_MATURITY_REDEEM"),
			Option(5, "STAKING_OPERATION_EARLY_REDEEM"),
			Option(6, "STAKING_OPERATION_MANUAL_REWARD"),
			Option(7, "STAKING_OPERATION_MANUAL_REDEEM"),
			Option(8, "STAKING_OPERATION_SUBSCRIBE_UNDO"),
		),
		Group("stakingOperationStatus", "质押操作状态",
			Option(1, "STAKING_OPERATION_PENDING"),
			Option(2, "STAKING_OPERATION_PROCESSING"),
			Option(3, "STAKING_OPERATION_SUCCEEDED"),
			Option(4, "STAKING_OPERATION_RETRYABLE_FAILED"),
			Option(5, "STAKING_OPERATION_MANUAL_REQUIRED"),
		),
		Group("stakingOperationStepStatus", "质押操作步骤状态",
			Option(0, "STAKING_STEP_NOT_REQUIRED"),
			Option(1, "STAKING_STEP_PENDING"),
			Option(2, "STAKING_STEP_SUCCEEDED"),
		),
		Group("stakingReconciliationStatus", "质押对账状态",
			Option(1, "STAKING_RECONCILIATION_MATCHED"),
			Option(2, "STAKING_RECONCILIATION_DIFF"),
			Option(3, "STAKING_RECONCILIATION_FAILED"),
		),
	}
}
...
func MarketOptions() []types.OptionsGroup {
	return []types.OptionsGroup{
		...
		EnumGroup("syncPriority", "K线优先级", market.SyncKlinePriority_SYNC_KLINE_PRIORITY_UNKNOWN.Descriptor()),
		EnumGroup("priceAlgorithm", "价格计算算法", market.PriceAlgorithm_PRICE_ALGORITHM_UNKNOWN.Descriptor()),
	}
}
