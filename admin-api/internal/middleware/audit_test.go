...
func TestAuditMiddlewareRedactsSecretsAndForcesTenantScope(t *testing.T) {
	...
	req := httptest.NewRequest(
		http.MethodPut,
		"/admin/member/users/10/status?accessToken=query-secret",
...
func TestAuditMiddlewareSystemAdminUsesTargetTenant(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/admin/option/trading-halts", strings.NewReader(`{"tenant_id":99}`))
...
func TestAuditMiddlewareSkipsReadOnlyAndExcludedRequests(t *testing.T) {
	...
	handler(httptest.NewRecorder(), httptest.NewRequest(http.MethodPost, "/admin/market/products", nil))
