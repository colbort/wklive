...
type (
	...
	CoinKlineModel interface {
		...
		FindRangeByMarketSymbol(ctx context.Context, market string, symbol string, startTs, endTs int64) ([]*CoinKline, error)
...
func (m *defaultCoinKlineModel) FindRangeByMarketSymbol(ctx context.Context, market string, symbol string, startTs, endTs int64) ([]*CoinKline, error) {
	...
	var list []*CoinKline
	err := m.conn.Find(ctx, &list, filter, options.Find().SetSort(bson.D{{Key: "ts", Value: 1}}))
	return list, err
}
