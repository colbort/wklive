CREATE TABLE `t_user` (
  ...
  `is_recharge` tinyint NOT NULL DEFAULT 2 COMMENT '是否充值；1是 2否',
  ...
  KEY `idx_tenant_account_type` (`tenant_id`, `account_type`, `status`),
  CONSTRAINT `chk_user_account_type` CHECK (`account_type` IN (1, 2, 3)),
...
CREATE TABLE `t_user_identity` (
  ...
  `verify_time` bigint NOT NULL DEFAULT 0 COMMENT '审核时间',
...
CREATE TABLE `t_user_security` (
  ...
  `google_enabled` tinyint NOT NULL DEFAULT 2 COMMENT 'Google2FA启用状态：1启用 2禁用',
