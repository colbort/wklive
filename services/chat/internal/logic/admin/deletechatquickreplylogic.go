...
// 删除快捷回复
func (l *DeleteChatQuickReplyLogic) DeleteChatQuickReply(in *chat.DeleteChatQuickReplyReq) (*chat.CommonResp, error) {
	if in.GetId() <= 0 {
	...
	if err == models.ErrNotFound {
		return &chat.CommonResp{Base: helper.ErrResp(404, "chat quick reply not found")}, nil
	}
	if err != nil {
		return &chat.CommonResp{Base: helper.ErrResp(500, err.Error())}, nil
	}
