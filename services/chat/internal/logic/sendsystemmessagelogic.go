...
func (l *SendSystemMessageLogic) SendSystemMessage(in *chat.SendSystemMessageReq) (*chat.InternalChatMessageResp, error) {
	...
	msg := newMessage(session, chat.ChatSenderType_CHAT_SENDER_TYPE_SYSTEM, 0, "system", "", normalizeMessageType(in.GetMessageType()), in.GetContent(), in.GetUrl(), in.GetFileName(), in.GetMimeType(), in.GetFileSize(), nil)
