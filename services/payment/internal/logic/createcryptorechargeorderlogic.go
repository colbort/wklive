...
func (l *CreateCryptoRechargeOrderLogic) CreateCryptoRechargeOrder(in *payment.CreateCryptoRechargeOrderReq) (*payment.CreateCryptoRechargeOrderResp, error) {
	if in.RechargeAmount <= 0 {
		return &payment.CreateCryptoRechargeOrderResp{Base: helper.GetErrResp(201, "recharge amount must be greater than zero")}, nil
	}
	if in.WalletType <= 0 || in.Coin == "" || in.ChainCode == 0 {
		return &payment.CreateCryptoRechargeOrderResp{Base: helper.GetErrResp(201, "invalid crypto recharge params")}, nil
	}
	...
	if err != nil {
		if errors.Is(err, models.ErrNotFound) {
			return &payment.CreateCryptoRechargeOrderResp{Base: helper.GetErrResp(201, i18n.Translate(i18n.CryptoRechargeAddressExpired, l.ctx))}, nil
		}
		return nil, err
	}
