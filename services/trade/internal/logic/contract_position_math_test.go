...
func TestCalculateContractTradeValues(t *testing.T) {
	linear, err := calculateContractTradeValues(int64(trade.ContractValueType_CONTRACT_VALUE_TYPE_LINEAR), decimal.NewFromInt(2), decimal.RequireFromString("0.001"), decimal.NewFromInt(50000))
	if err != nil {
		t.Fatal(err)
	}
	...
	if fee := calculateContractFee(linear, decimal.RequireFromString("0.0005")); !fee.Equal(decimal.RequireFromString("0.05")) {
		t.Fatalf("linear fee = %s, want 0.05", fee)
	}

	inverse, err := calculateContractTradeValues(int64(trade.ContractValueType_CONTRACT_VALUE_TYPE_INVERSE), decimal.NewFromInt(100), decimal.NewFromInt(100), decimal.NewFromInt(50000))
	if err != nil {
		t.Fatal(err)
	}
	...
	if fee := calculateContractFee(inverse, decimal.RequireFromString("0.0005")); !fee.Equal(decimal.RequireFromString("0.0001")) {
		t.Fatalf("inverse fee = %s, want 0.0001", fee)
	}
	if margin, err := calculateContractMargin(inverse, 10); err != nil || !margin.Equal(decimal.RequireFromString("0.02")) {
