...
| 2026-07-29 | P1-01 Authority Registry 管理入口与动态下拉 | 通过 | Proto、Market RPC 和 Admin API 新增 Authority 查询/设置；所有注册表 SQL 均位于 Market models。创建时规范化 Authority/producer/type，更新使用 version 乐观锁，名称与 producer 不可变；停用或移除类型前由 model 扫描激活公式，真实尝试停用 `market-ws` 被 `authority is referenced by active price formulas` 拒绝，状态/version 保持 1/0。Admin UI 价格公式页不再硬编码三个 Authority，输出按目标类型筛选，成分类型取自注册表。RBAC 菜单 483/484 经 migration 50 落库并刷新管理员单条权限缓存；ALL、FINAL_QUOTE、MARK 三类查询分别返回 3、2、1 条正确记录。Market/Admin 全量 Go 测试、Admin UI type-check 和补丁检查通过；镜像 `a5d626ad…81734d`、`b6a80f1e…c1949d` Healthy。未创建虚假第三来源。详见 `evidence/2026-07-29-preprod/authority-registry-management-report.md` |
| 2026-07-29 | P1-01 独立供应商硬门禁 | 通过（负向门禁）；待真实三供应商正向验收 | Authority Registry 新增不可为空且创建后不可修改的 `provider_code`，迁移 51 将 `market-ws`、`market-rest` 同时归为 `ITICK`，`price-engine` 归为 `PRICE_ENGINE`。INDEX/DELIVERY 创建同时要求 Authority 和 provider 均唯一，不能再用同一供应商的 WebSocket/REST 通道凑三源；生产就绪模型同时核对启用 Authority 数和 `COUNT(DISTINCT provider_code)`。运行态管理接口返回三条记录及供应商映射，以 `market-ws + market-rest + 第三 Authority` 创建 INDEX 被 `INDEX and DELIVERY components must use independent providers` 拒绝，公式落库数为 0。新 Market/Admin 镜像 `5513ea58…aaaf`、`b2899d87…d641` Healthy；迁移、全量 Go 测试、Admin UI type-check/Prettier 和预生产 14 项负向门禁通过。详见 `evidence/2026-07-29-preprod/authority-registry-management-report.md` |
...
| 2026-07-29 | iTick 凭据外置化 | 通过；生产凭据审批仍未通过 | 将受版本控制 YAML 中的 iTick Token 改为 `__ITICK_TOKEN__` 占位符，现有值迁移至 Git 忽略、权限 `0600` 的 `deploy/secrets/market_token`；Compose 以 Docker Secret 挂载，`config-seed` 校验后仅在写入 Etcd 时注入，缺失 Secret 明确失败。Compose 渲染、shell 语法、缺失凭据负向测试及仓库原值扫描通过；实际重新导入 17 个 Etcd 配置键并重启 iTick 后服务 Healthy，最近两分钟 `market-ws FINAL_QUOTE` 105 条、INDEX/MARK/FUNDING 各 120 条，近三分钟无鉴权或引擎错误。此项不把真实供应商凭据和数据许可审批改为通过。详见 `evidence/2026-07-29-preprod/market-credential-externalization-report.md` |
