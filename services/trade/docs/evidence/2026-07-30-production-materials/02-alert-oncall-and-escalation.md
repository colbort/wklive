...
- 已创建 `contract_oncall`、`production_reviewer` 和 `production_approver`
  三个职责分离账号；六个生产职责账号均已完成真实 Admin 登录验证；
- `contract_oncall` 具备三类告警所需读取权限且无后台写权限，候选升级目标已经映射为
  L1 值班、L2 复核、L3 审批。
...
系统责任账号和时限已经配置，但账号背后的真实人员排班仍需管理人员确认。技术实现与
L1 运行态验收已完成，仅凭系统账号不能证明 7×24 人员响应。
