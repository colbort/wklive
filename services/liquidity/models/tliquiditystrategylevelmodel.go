...
type (
	...
	TLiquidityStrategyLevelModel interface {
		...
		FindList(ctx context.Context, configID int64, enabledOnly bool) ([]*TLiquidityStrategyLevel, error)
		CountEnabled(ctx context.Context, configID int64) (int64, error)
		Replace(ctx context.Context, configID int64, levels []LiquidityStrategyLevelInput, now int64) error
	}
...
func (m *customTLiquidityStrategyLevelModel) Replace(ctx context.Context, configID int64, levels []LiquidityStrategyLevelInput, now int64) error {
	oldRows, err := m.FindList(ctx, configID, false)
	if err != nil {
		return err
	}
	err = m.TransactCtx(ctx, func(ctx context.Context, session sqlx.Session) error {
		if _, err := session.ExecCtx(ctx, fmt.Sprintf("DELETE FROM %s WHERE config_id = ?", m.table), configID); err != nil {
			return err
		}
	...
	for _, row := range oldRows {
		keys = append(keys,
			...
			fmt.Sprintf("%s%v:%v", cacheTLiquidityStrategyLevelConfigIdLevelNoPrefix, row.ConfigId, row.LevelNo),
		)
	}
	if len(keys) > 0 {
