...
- 最后交易边界后禁止新交易、撮合和人工恢复，合约保持 `PAUSED`；暂停交易不剥夺截止前的指令权，
  `PENDING/EXPIRED/SETTLED/OFFLINE` 不接受新提交。
...
| 检查 | 预期 | 事实/数量 | 证据 |
| --- | --- | --- | --- |
| 服务 UTC / NTP UTC / 偏差 | 在批准阈值内 | `[VALUE]` | `[LINK/HASH]` |
| 延迟消息 expected last trade / 数据库 last trade | 完全一致 | `[VALUE]` | `[LINK/HASH]` |
| 合约状态 | `PAUSED` | `[STATUS]` | `[LINK/HASH]` |
| FUNDING / PENDING / PART_FILLED | `0 / 0 / 0` | `[N] / [N] / [N]` | `[LINK/HASH]` |
| CANCELING / EXPIRING | `0 / 0` | `[N] / [N]` | `[LINK/HASH]` |
| 最后交易撤单原因 / 数量 | `CONTRACT_LAST_TRADE_ENDED / [N]` | `[VALUE]` | `[LINK/HASH]` |
| 冻结释放指令未终态 / 未对账 | `0 / 0` | `[N] / [N]` | `[LINK/HASH]` |
| 新交易拒绝 / 恢复交易拒绝 | 均拒绝 | `[RESULT]` | `[LINK/HASH]` |
| 到期行权 / 结算价锁定 / 结算批次 / 交割 | `0 / 0 / 0 / 0` | `[N] / [N] / [N] / [N]` | `[LINK/HASH]` |
| 重复、迟到、旧身份消息重放 | 零新增资金动作、无提前到期 | `[RESULT]` | `[LINK/HASH]` |

任一活动/处理中订单、未终态释放、时钟异常或提前清算非零，立即置为 `EXCEPTION`；不得通过修改
`last_trade_time`、`expire_time` 或直接恢复 `TRADING` 绕过。
