...
“材料完成”不等于“生产阻断关闭”。只有实际归档副本为 `APPROVED`、附件哈希匹配、终态校验确认
无占位符/空表格/未决状态且生产 readiness通过，才能在发布签署单中关闭对应阻断。
...
| 产品公告与用户运营 | 系统能力边界、通知/FAQ/客服底稿、逐合约合集、31项固定材料哈希记录及三方集合校验器 | 具体合约参数、法律文本、触达计划、联系人、无占位符发布副本、目标环境/公告导出及完整合约集合 | `option-product-operations-pack.md`、`option-operations-input-checklist.md`、`option-contract-launch-checklist.md`、`templates/option-contract-launch-bundle.md`、`templates/option-contract-set-reconciliation.md`、`../monitoring/option-launch-bundle-verify.sh` | MATERIAL_READY / EXTERNAL_EXECUTION | 产品参数、法律文本、渠道/人员和发布时间；生产门禁逐份验证固定材料文件/SHA/终态并要求审批、目标环境和公告/客户端三方集合完全相同 |
...
| 可选产品运行时范围 | `ProductScope`零值关闭；上市、卖方开仓、组合单、公开行情、美式行权和MMP入口门禁；关闭后保留风险降低路径 | 渲染YAML与发布声明逐项一致并固定哈希；MMP、美式启用时专项目标环境报告 | `../etc/option.yaml`、`../monitoring/option-production-readiness.env.example`、`templates/option-mmp-readiness.md`、`templates/option-american-exercise-readiness.md` | MATERIAL_READY / EXTERNAL_EXECUTION | 目标环境Option渲染YAML、逐项布尔值、MMP-PRE-001～008、AMER-PRE-001～010和六方签署 |
...
- 所有生产门禁引用的报告除文件存在和哈希外，还校验机器可读批准状态，并由
  `option-evidence-finalization-verify.sh`拒绝表格占位符、空单元格、未决选择、OPEN/DRAFT行和未勾选项。
- 生产readiness声明以仓库示例为精确键集合schema，`option-readiness-attestation-verify.sh`拒绝重复、
  未知、缺失键、非规范赋值和占位路径后才允许读取tenant、release、开关及证据SHA。
- 未批准能力保持运行时功能开关、API、权限、公告和销售口径关闭，不以人工流程绕过；生产门禁声明必须与
  哈希绑定的Option `ProductScope`渲染配置逐项相同。
