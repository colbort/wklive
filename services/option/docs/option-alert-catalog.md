...
| OPT-A008 | SEV-2 | outbox/inbox 最老未完成记录超过 60 秒或连续 3 分钟增长 | 保护依赖状态；禁止到期推进 | 值班、Option 技术 | 数量/最早时间指标和超龄规则已实现；增长规则、生产抓取/通知待接入 |
| OPT-A009 | SEV-2 | 待处理主动行权超过 60 秒；到期前 30 分钟仍非 0 | 到期流程等待；必要时关闭主动行权入口 | 值班、运营、风控 | 数量/最早时间指标和超龄规则已实现；到期前动态规则、生产抓取/通知待接入 |
| OPT-A010 | SEV-2 | 到期窗口结束后 60 秒仍未锁定结算价 | 保持等待，不允许用当前价替代 | 值班、行情、清算 | 代码保持等待，指标待接入 |
| OPT-A011 | SEV-1 | 已发布结算价样本数低于合约最小值、来源不符或窗口外 | 阻止结算批次 | 技术负责人、清算、合规 | 代码校验和审批工作台已实现，外部指标/通知待接入 |
...
| OPT-A015 | SEV-1 | 日终资金守恒差额非 0 | 暂停次日相关产品开放 | 技术、清算、财务、合规 | 日终流程待自动化 |
| OPT-A016 | SEV-3 | 管理员修改经济字段被拒绝或重复尝试 | 无；保留安全审计 | 值班、安全、风控 | 应用日志可查，审计指标待接入 |
...
| OPT-A020 | SEV-2 | kill switch 启用后 30 秒仍存在该用户活动订单，或撤单/资金释放失败 | 新单持续拒绝，重复执行幂等撤单 | 值班、风控、清算 | kill switch/撤单、残单租户指标和规则已实现；生产抓取/通知待接入 |
...
| OPT-A025 | SEV-2；版本不一致升 SEV-1 | 存在组合持仓但无当前有效已批准参数；风险账户配置版本落后当前版本两个扫描周期；下单与扫描解析版本不一致 | 拒绝组合卖单，风险账户转 `RESTRICTED`；不得回退默认参数 | 值班、风控模型、Option 技术 | 准入/限制和快照字段已实现，指标/通知待接入 |
| OPT-A026 | SEV-2；已有扣款或逾期升 SEV-1 | 实物交割单元进入 `CURE_REQUIRED/MANUAL_REVIEW/DEFAULTED`，或补资截止后仍未完成 | 阻止该单元入账；健康单元继续；逾期停止自动重试 | 值班、清算、风控、运营、Option/Asset | 状态保护、不可变事件、租户指标和规则已实现；生产抓取/通知路由待接入 |
| OPT-A027 | SEV-2；生效后未完成或人工态升 SEV-1 | 公司行动到达 `effective_time` 后仍未完成、任一映射连续失败、进入 `MANUAL_REVIEW`，或源/后继合约提前恢复交易 | 源合约保持停牌，后继合约禁止上市，相关钱包风险账户转 `RESTRICTED` | 值班、市场运营、风控、清算、合规、Option/Asset | 状态保护和后台进度已实现，外部指标/通知路由待接入 |
| OPT-A028 | SEV-3；谱系不一致升 SEV-1 | 合约系列待复核超过24小时；已生成数量不等于预计数量；谱系指向缺失合约；上市批准前合约已非 PENDING | 不自动上市；数量/谱系不一致时冻结该系列全部合约的上市审批 | 值班、产品运营、风控、Option 技术 | 原子事务、独立上市门禁和后台明细已实现，外部指标/通知待接入 |
| OPT-A029 | SEV-2；跨租户/错误状态泄漏升 SEV-1 | 公开链 Call/Put 缺腿；多空 `HOLDING` OI 不等；统计/盘口与事实表抽样不一致；响应超过批准 TTL/SLA | 标记行情延迟或暂时下线对应链/盘口；保持交易事实不变；跨租户泄漏立即关闭公开入口 | 值班、行情运营、风控、Option 技术、安全 | 质量字段和事实源查询已实现，指标/CDN探测/通知待接入 |
...
- `wklive_option_operations_amount{tenant_id,category,coin}`：保险账本、兜底负债和未解决缺口金额；
...
`monitoring/option-operations-alert-rules.yml`，覆盖 OPT-A001～A009/A012～A014/
A017～A024/A026（OPT-A003 仅缺失/未来时间）。交易控制/MMP 只输出租户和固定类别：阈值聚合在数据库按
用户/合约/组计算，具体身份从不可变审计与工作台下钻，禁止作为 Prometheus 标签。
事件增长、到期前动态窗口、价格带请求比例分母和资金字段冲突仍须在生产规则/演练中补齐。
时间窗口查询索引由 `20260731_zr_option_operations_monitoring_indexes.sql` 安装。
