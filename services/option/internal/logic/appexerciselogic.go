...
func (l *AppExerciseLogic) AppExercise(in *option.AppExerciseReq) (*option.AppExerciseResp, error) {
	...
	if err != nil {
		if errors.Is(err, models.ErrNotFound) {
			return &option.AppExerciseResp{Base: helper.ErrResp(i18n.PositionNotFound, i18n.Translate(i18n.PositionNotFound, l.ctx))}, nil
		}
		return nil, err
	}
	if position.TenantId != tenantId || position.UserId != userId || position.AccountId != in.AccountId {
		return &option.AppExerciseResp{Base: helper.ErrResp(i18n.NoPermissionOperatePosition, i18n.Translate(i18n.NoPermissionOperatePosition, l.ctx))}, nil
	}
	...
	if in.ContractId != 0 && position.ContractId != in.ContractId {
		return &option.AppExerciseResp{Base: helper.ErrResp(i18n.ContractPositionMismatch, i18n.Translate(i18n.ContractPositionMismatch, l.ctx))}, nil
	}

	contract, err := l.svcCtx.OptionContractModel.FindOne(l.ctx, position.ContractId)
	if err != nil {
		if errors.Is(err, models.ErrNotFound) {
			return &option.AppExerciseResp{Base: helper.ErrResp(i18n.ContractNotFound, i18n.Translate(i18n.ContractNotFound, l.ctx))}, nil
		}
		return nil, err
	}
	...
	if contract.ExerciseStyle == int64(option.ExerciseStyle_EXERCISE_STYLE_EUROPEAN) && now < contract.ExpireTime {
		return &option.AppExerciseResp{Base: helper.ErrResp(i18n.EuropeanOptionNotExpired, i18n.Translate(i18n.EuropeanOptionNotExpired, l.ctx))}, nil
	}
	market, err := l.svcCtx.OptionMarketModel.FindOneByTenantIdContractId(l.ctx, tenantId, contract.Id)
	if err != nil {
		if errors.Is(err, models.ErrNotFound) {
			return &option.AppExerciseResp{Base: helper.ErrResp(i18n.MarketNotFound, i18n.Translate(i18n.MarketNotFound, l.ctx))}, nil
		}
		return nil, err
	}
