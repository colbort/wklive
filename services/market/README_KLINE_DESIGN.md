...
> 本文以 `services/market` 当前代码为准，说明历史数据同步、实时 `1m` 聚合、每 5 分钟校正及高周期派生的实际调用链。未实现或仍有风险的部分单独列在文末。
...
`MarketManager.rebuildActiveProducts` 从 MySQL 全量重建：

| Key | 类型 | 当前用途 |
| --- | --- | --- |
| `market:v1:active_products` | Set | 活跃产品 ID |
| `market:v1:product:{id}` | Hash | `category_code/market/symbol/exchange` |
...
```text
SMEMBERS market:v1:active_products
```
...
```text
market:v1:kline:{category}:{market}:{symbol}:{interval}
```
...
| 数据 | Key | TTL |
| --- | --- | --- |
| Quote | `market:v1:quote:{category}:{market}:{symbol}` | 30 分钟 |
| Tick | `market:v1:tick:{category}:{market}:{symbol}` | 30 分钟 |
| Depth | `market:v1:depth:{category}:{market}:{symbol}` | 5 分钟 |
| Kline | `market:v1:kline:{category}:{market}:{symbol}:{interval}` | 24 小时 |

另外保存：

- `market:v1:kline:building:{category}:{market}:{symbol}:{ts}`：未闭合 1m 桶，TTL 使用 `buildingBucketTtlMinutes`；
- `market:v1:kline:baseline:{productKey}`：股票累计成交量基线；
...
| WS 订阅与消息处理 | `internal/market/client/market_ws_client.go` |
| 活跃产品和订阅构建 | `internal/market/client/market_manager.go` |
