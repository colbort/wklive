...
func (c *Client) Get(ctx context.Context, rawURL string) (*http.Response, error) {
	if c == nil {
		return nil, fmt.Errorf("market REST client is not configured")
	}
	if c.limiter != nil {
	...
	return nil, fmt.Errorf("market REST returned status=%d body=%s", resp.StatusCode, strings.TrimSpace(string(body)))
}
