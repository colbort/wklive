...
const (
	activeProductsKey  = "market:v1:active_products"
...
func (w *SyncKlinesWorker) loadActiveProducts() ([]*models.TMarketProduct, error) {
	if w.svcCtx.DataCache != nil {
		...
		if err == nil && len(members) > 0 {
			...
			if len(ids) > 0 {
				return w.svcCtx.MarketProductModel.FindByIds(w.ctx, ids)
			}
		}
		if err != nil {
...
func (w *SyncKlinesWorker) reconcileBatch(apiURL, token string, group *reconcileGroup, products []*models.TMarketProduct) error {
	...
	windowBars := 30
	if w.svcCtx.MarketRuntimeConfig != nil {
		windowBars = int(w.svcCtx.MarketRuntimeConfig.ReconcileWindowBars)
	}
	if windowBars <= 0 {
		windowBars = 30
	}
...
func (w *SyncKlinesWorker) reconcileProduct(group *reconcileGroup, product *models.TMarketProduct,
	...
	progress, err := w.svcCtx.MarketKlineSyncProgressModel.FindOrCreate(w.ctx, group.category, group.market, product.Symbol, interval)
	if err != nil {
		return err
	}
	if latestTs == 0 {
	...
	if err := w.svcCtx.MarketKlineSyncProgressModel.UpdateSyncSuccess(w.ctx, progress.Id, "reconcile", latestTs,
...
func validClosedKline(item MarketKlineItem, lastClosed, intervalMs int64) bool {
...
func (w *SyncKlinesWorker) getBatchKlines(ctx context.Context, apiURL, token, category, market, exchange string,
	codes []string, kType int32, limit int) (map[string][]MarketKlineItem, error) {
...
func (w *SyncKlinesWorker) toCoinKline(job KlineJob, interval string, item MarketKlineItem) *models.CoinKline {
...
func (w *SyncKlinesWorker) updateTaskStatus(taskNo string, status int64, message string) error {
	return w.svcCtx.MarketSyncTaskModel.UpdateStatusByTaskNo(
		w.ctx,
		taskNo,
		status,
		message,
