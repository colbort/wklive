...
func (a *TickAggregator) persist(ctx context.Context, key tickBucketKey, bucket *tickBucket, productKey string) {
	...
	pipe.Set(ctx, "market:v1:kline:baseline:"+productKey, base, a.stateTTL)
	_, _ = pipe.Exec(ctx)
}

func (a *TickAggregator) bucketRedisKey(key tickBucketKey) string {
	return fmt.Sprintf("market:v1:kline:building:%s:%s:%s:%d", key.category, key.market, key.symbol, key.ts)
}
...
func (a *TickAggregator) restore(ctx context.Context) {
	...
	for _, pattern := range []string{"market:v1:kline:building:*", "market:v1:kline:baseline:*"} {
		var cursor uint64
		for {
