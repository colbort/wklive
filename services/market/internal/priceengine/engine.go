...
func (e *Engine) evaluate(ctx context.Context, f *models.TItickPriceFormula, target int64) error {
	...
	if market.PriceAlgorithm(f.Algorithm) == market.PriceAlgorithm_PRICE_ALGORITHM_PREMIUM_RATE && len(components) != 2 {
		return errors.New("PREMIUM_RATE requires exactly mark and index components")
	}
	if market.PriceAlgorithm(f.Algorithm) == market.PriceAlgorithm_PRICE_ALGORITHM_INDEX_BASIS {
	...
	for componentIndex, c := range components {
		...
		lookupTarget := target
		if market.PriceAlgorithm(f.Algorithm) == market.PriceAlgorithm_PRICE_ALGORITHM_INDEX_BASIS && componentIndex == 2 {
	...
	if market.PriceAlgorithm(f.Algorithm) == market.PriceAlgorithm_PRICE_ALGORITHM_INDEX_BASIS {
		...
		price, err = Calculate(market.PriceAlgorithm(f.Algorithm), inputs)
	}
	if err != nil {
		return err
	}
...
func ReplayEvaluationAudit(raw []byte) (decimal.Decimal, error) {
	...
	algorithm := market.PriceAlgorithm(audit.Algorithm)
	expectedAccepted, expectedRejected := deduplicateInputsWithAudit(
		append([]Input(nil), audit.AllInputs...),
	)
	if algorithm != market.PriceAlgorithm_PRICE_ALGORITHM_INDEX_BASIS {
		var deviationRejected []Input
