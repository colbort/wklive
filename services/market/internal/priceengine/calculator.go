...
func Calculate(algorithm market.PriceAlgorithm, inputs []Input) (decimal.Decimal, error) {
	if len(inputs) == 0 {
	...
	switch algorithm {
	case market.PriceAlgorithm_PRICE_ALGORITHM_WEIGHTED_MEAN:
	...
	case market.PriceAlgorithm_PRICE_ALGORITHM_MEDIAN:
	...
	case market.PriceAlgorithm_PRICE_ALGORITHM_PREMIUM_RATE:
		if len(inputs) != 2 {
	...
	case market.PriceAlgorithm_PRICE_ALGORITHM_INDEX_BASIS:
