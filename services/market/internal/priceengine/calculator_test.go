...
func TestCalculate(t *testing.T) {
	p, e := Calculate(market.PriceAlgorithm_PRICE_ALGORITHM_WEIGHTED_MEAN, []Input{{Price: decimal.NewFromInt(100), Weight: decimal.NewFromInt(1)}, {Price: decimal.NewFromInt(110), Weight: decimal.NewFromInt(3)}})
	if e != nil || !p.Equal(decimal.RequireFromString("107.5")) {
		t.Fatalf("weighted=%s err=%v", p, e)
	}
	p, e = Calculate(market.PriceAlgorithm_PRICE_ALGORITHM_MEDIAN, []Input{{Price: decimal.NewFromInt(3)}, {Price: decimal.NewFromInt(1)}, {Price: decimal.NewFromInt(2)}})
...
func TestPremiumRateCanBeNegative(t *testing.T) {
	p, err := Calculate(market.PriceAlgorithm_PRICE_ALGORITHM_PREMIUM_RATE, []Input{{Price: decimal.NewFromInt(99)}, {Price: decimal.NewFromInt(100)}})
