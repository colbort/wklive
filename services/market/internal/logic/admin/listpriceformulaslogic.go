...
func (l *ListPriceFormulasLogic) ListPriceFormulas(in *market.ListPriceFormulasReq) (*market.ListPriceFormulasResp, error) {
	...
	data := make([]*market.PriceFormulaData, 0, len(rows))
	...
	return &market.ListPriceFormulasResp{Base: pageutil.Base(in.Page.Cursor, in.Page.Limit, len(rows), count, lastID), Data: data}, nil
}
