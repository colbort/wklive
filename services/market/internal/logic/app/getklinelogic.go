...
func (l *GetKlineLogic) GetKline(in *market.GetKlineReq) (*market.GetKlineResp, error) {
	...
	if model == nil {
		return &market.GetKlineResp{Base: helper.OkResp(), Data: []*market.Kline{}}, nil
	}
	endTs := in.EndTs
	if endTs <= 0 {
	...
	data := make([]*market.Kline, 0, len(result))
	for _, item := range result {
		data = append(data, helpers.ToKlineProto(in.KType, item))
	}
	return &market.GetKlineResp{Base: helper.OkResp(), Data: data}, nil
}

func (l *GetKlineLogic) getYearKlines(in *market.GetKlineReq) (*market.GetKlineResp, error) {
	endTs := in.EndTs
	if endTs <= 0 {
		endTs = time.Now().UnixMilli() + 1
	}
	...
	data := make([]*market.Kline, 0, min(int(limit), len(years)))
...
func aggregateQueryKlines(in *market.GetKlineReq, ts int64, list []*models.CoinKline) *models.CoinKline {
