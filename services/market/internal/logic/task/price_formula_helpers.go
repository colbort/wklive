...
func normalizePriceFormulaReq(in *market.CreatePriceFormulaReq) ([]priceengine.Component, error) {
	...
	if in.Algorithm != market.PriceAlgorithm_PRICE_ALGORITHM_WEIGHTED_MEAN &&
		in.Algorithm != market.PriceAlgorithm_PRICE_ALGORITHM_MEDIAN &&
		in.Algorithm != market.PriceAlgorithm_PRICE_ALGORITHM_PREMIUM_RATE &&
		in.Algorithm != market.PriceAlgorithm_PRICE_ALGORITHM_INDEX_BASIS {
		return nil, errors.New("unsupported price formula algorithm")
	}
	if len(in.Components) == 0 ||
		(in.Algorithm == market.PriceAlgorithm_PRICE_ALGORITHM_PREMIUM_RATE && len(in.Components) != 2) ||
		(in.Algorithm == market.PriceAlgorithm_PRICE_ALGORITHM_INDEX_BASIS && len(in.Components) != 2 && len(in.Components) != 3) {
	...
	if in.SnapshotKind == "INDEX" {
		if len(components) < 3 || in.MinInputCount < 3 ||
			(in.Algorithm != market.PriceAlgorithm_PRICE_ALGORITHM_MEDIAN &&
				in.Algorithm != market.PriceAlgorithm_PRICE_ALGORITHM_WEIGHTED_MEAN) {
...
func toPriceFormulaProto(row *models.TItickPriceFormula) *market.PriceFormulaData {
	...
	result := &market.PriceFormulaData{Id: row.Id, FormulaNo: row.FormulaNo, Authority: row.Authority, SnapshotKind: row.SnapshotKind, CategoryCode: row.CategoryCode, Market: row.Market, Symbol: row.Symbol, Algorithm: market.PriceAlgorithm(row.Algorithm), FormulaVersion: row.FormulaVersion, MaxLookbackMs: row.MaxLookbackMs, MaxDeviationBps: row.MaxDeviationBps, MinInputCount: row.MinInputCount, IntervalMs: row.IntervalMs, LastTargetTime: row.LastTargetTime, Status: int32(row.Status), Version: row.Version, CreateTimes: row.CreateTimes, UpdateTimes: row.UpdateTimes}
	for _, component := range components {
		result.Components = append(result.Components, &market.PriceFormulaComponent{Authority: component.Authority, SnapshotKind: component.Kind, CategoryCode: component.CategoryCode, Market: component.Market, Symbol: component.Symbol, Weight: component.Weight})
	}
	return result
}
