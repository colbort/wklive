...
CREATE TABLE `t_itick_snapshot_outbox` (
  ...
  KEY `idx_snapshot_outbox_claim` (`status`,`claimed_at`,`id`),
...
CREATE TABLE `t_itick_price_formula` (
  ...
  `algorithm` TINYINT NOT NULL COMMENT '1 weighted mean,2 median,3 premium rate,4 index plus bounded basis', `formula_version` VARCHAR(64) NOT NULL,
  `components` JSON NOT NULL, `max_lookback_ms` BIGINT NOT NULL DEFAULT 30000,
  `max_deviation_bps` INT NOT NULL DEFAULT 0, `min_input_count` INT NOT NULL DEFAULT 1,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='market市场节假日及特殊交易日';
