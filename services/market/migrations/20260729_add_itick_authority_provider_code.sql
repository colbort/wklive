...
SET @dbinit_exists := (
  ...
  WHERE table_schema = DATABASE()
    AND table_name = 't_market_authority_registry'
    AND column_name = 'provider_code'
);
SET @dbinit_sql := IF(
  @dbinit_exists = 0,
  'ALTER TABLE `t_market_authority_registry` ADD COLUMN `provider_code` VARCHAR(32) NOT NULL DEFAULT '''' COMMENT ''独立数据供应商标识；同一供应商不同传输通道必须相同'' AFTER `authority`',
  'SELECT 1'
);
...
SET `provider_code` = CASE
  WHEN `authority` IN ('market-ws','market-rest') THEN 'ITICK'
...
SET @dbinit_sql := IF(
  @dbinit_exists > 0,
  'ALTER TABLE `t_market_authority_registry` DROP CHECK `chk_authority_registry`',
  'SELECT 1'
);
PREPARE dbinit_stmt FROM @dbinit_sql;
EXECUTE dbinit_stmt;
...
ALTER TABLE `t_market_authority_registry`
