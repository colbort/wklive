...
type (
	...
	SysVerificationCodeRecord struct {
		...
		Channel      int64          `db:"channel"`       // 发送渠道：0未知 1邮箱 2手机短信
		Target       string         `db:"target"`        // 发送目标：邮箱或手机号
		Scene        int64          `db:"scene"`         // 业务场景：0未知 1注册 2登录 3重置密码 4绑定邮箱 5绑定手机 6修改密码 7提现 100测试
		Code         string         `db:"code"`          // 验证码
		Status       int64          `db:"status"`        // 发送状态：0未知 1成功 2失败
