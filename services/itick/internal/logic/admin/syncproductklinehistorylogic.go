...
func (l *SyncProductKlineHistoryLogic) SyncProductKlineHistory(in *itick.SyncProductKlineHistoryReq) (*itick.SyncProductKlineHistoryResp, error) {
	if strings.TrimSpace(l.svcCtx.Config.Itick.ApiUrl) == "" {
		return &itick.SyncProductKlineHistoryResp{Base: helper.ErrResp(i18n.ApiURLRequired, i18n.Translate(i18n.ApiURLRequired, l.ctx))}, nil
	}
	if strings.TrimSpace(l.svcCtx.Config.Itick.Token) == "" {
		return &itick.SyncProductKlineHistoryResp{Base: helper.ErrResp(i18n.ApiTokenRequired, i18n.Translate(i18n.ApiTokenRequired, l.ctx))}, nil
	}
	...
	if interval == "" || strings.TrimSpace(in.CategoryCode) == "" || strings.TrimSpace(in.Market) == "" || strings.TrimSpace(in.Symbol) == "" {
		return &itick.SyncProductKlineHistoryResp{Base: helper.ErrResp(i18n.ParamError, i18n.Translate(i18n.ParamError, l.ctx))}, nil
	}
	...
	return &itick.SyncProductKlineHistoryResp{Base: helper.OkResp(), SyncedCount: int64(result.NewCount)}, nil
}
