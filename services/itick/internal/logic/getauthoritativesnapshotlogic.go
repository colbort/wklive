...
func (l *GetAuthoritativeSnapshotLogic) GetAuthoritativeSnapshotInternal(in *itick.GetAuthoritativeSnapshotInternalReq) (*itick.GetAuthoritativeSnapshotInternalResp, error) {
	if in == nil {
		return nil, fmt.Errorf("invalid authoritative snapshot query")
	}
	data, err := l.getAuthoritativeSnapshot(in.Authority, in.CategoryCode, in.Market, in.Symbol, in.TargetTime, in.MaxLookbackMs, in.SnapshotKind)
	...
	return &itick.GetAuthoritativeSnapshotInternalResp{Base: helper.OkResp(), Data: data}, nil
}
