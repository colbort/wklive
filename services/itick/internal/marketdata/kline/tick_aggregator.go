...
func NewTickAggregator(writer *klinewriter.BatchWriter, lateness time.Duration) *TickAggregator {
	if lateness <= 0 {
		lateness = 90 * time.Second
	}
	return &TickAggregator{writer: writer, lateness: lateness, buckets: make(map[tickBucketKey]*tickBucket),
...
func (a *TickAggregator) Add(_ context.Context, msg types.ClientMessage, tick *types.TickPayload) {
	...
	a.seen[fingerprint] = now + int64(a.lateness) + minuteMillis
