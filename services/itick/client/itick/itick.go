...
func NewItick(cli zrpc.Client) Itick {
	return &defaultItick{
		cli: cli,
	}
}
...
// Reads an authoritative snapshot from the permanent archive at or before
func (m *defaultItick) GetAuthoritativeSnapshot(ctx context.Context, in *GetAuthoritativeSnapshotReq, opts ...grpc.CallOption) (*GetAuthoritativeSnapshotResp, error) {
	client := itick.NewItickClient(m.cli.Conn())
