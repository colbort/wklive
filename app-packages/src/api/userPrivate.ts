import { clearAccessToken, clearRefreshToken, http, setTenantId } from './http'
...
export function apiLogout(): Promise<RespBase> {
  return http.post('/user/logout').then((res: { data: any }) => {
    clearAccessToken()
    clearRefreshToken()
    return res.data
  })
...
export function apiGetProfile(): Promise<RespBase & GetProfileResp> {
  return http.get<RespBase & GetProfileResp>('/user/profile').then((res: { data: any }) => {
    const data = res.data
...
export function apiUpdateProfile(params: UpdateProfileReq): Promise<RespBase & UpdateProfileResp> {
  return http.put('/user/profile', params).then((res: { data: any }) => {
    const data = res.data
    return data
  })
}
...
export function apiDeleteBank(params: DeleteBankReq): Promise<RespBase> {
  return http.delete(buildPath('/user/banks/:id', { id: params.id })).then((res: { data: any }) => res.data)
}

export function apiSetDefaultBank(params: SetDefaultBankReq): Promise<RespBase> {
  return http.put(buildPath('/user/banks/:id/default', { id: params.id })).then((res: { data: any }) => res.data)
}
