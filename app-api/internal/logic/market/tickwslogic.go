...
func (l *TickWsLogic) restartSubscriptionStream(runtime *tickWsRuntime) error {
	...
	reqTopics := make([]*market.SubscribeTopic, 0, len(topics))
	for _, topic := range topics {
		reqTopics = append(reqTopics, &market.SubscribeTopic{
			Topic:        topic.Topic,
	...
	stream, err := l.svcCtx.MarketCli.SubscribeStream(streamCtx, &market.SubscribeRequest{Topics: reqTopics})
	if err != nil {
		cancelStream()
		return err
	}
