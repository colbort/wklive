...
import { tradeService, type ContractPosition } from '@/services'
...
const rows = ref<ContractPosition[]>([])
const detailVisible = ref(false)
const detailData = ref<ContractPosition | null>(null)
...
const loadCurrent = async () => {
  loading.value = true
  try {
    rows.value = pickList(await tradeService.listPositions(currentQuery))
  } finally {
    loading.value = false
  }
}
...
const showDetail = async (row: ContractPosition) => {
  detailData.value =
    (await tradeService.getPosition({ tenantId: row.tenantId, id: row.id })).data || row
