  ...
  <div class="module-page">
    ...
    <el-card shadow="never" class="table-card">
      <el-table v-loading="loading" :data="rows" stripe>
        ...
        <el-table-column prop="feeAsset" :label="t('trade.feeAsset')" min-width="110" />
        <el-table-column prop="feeAmount" :label="t('trade.feeAmount')" min-width="130" />
...
import { apiTradeListFundingSettlements } from '@/api/trade'
import type { ContractFundingSettlement } from '@/services'
...
const rows = ref<ContractFundingSettlement[]>([])
const loading = ref(false)
async function load() {
  loading.value = true
  try {
    const res = await apiTradeListFundingSettlements({
      ...query,
