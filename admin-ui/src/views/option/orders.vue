...
const rows = ref<OptionOrder[]>([])
const detailVisible = ref(false)
const detailData = ref<OptionOrderDetail | OptionOrder | null>(null)
...
const loadCurrent = async () => {
  loading.value = true
  try {
    const res = await optionService.listOrders({
      ...query,
...
const showDetail = async (row: OptionOrder) => {
  detailData.value =
    (await optionService.getOrder({ tenantId: row.tenantId, id: row.id, orderNo: row.orderNo }))
      .data || row
