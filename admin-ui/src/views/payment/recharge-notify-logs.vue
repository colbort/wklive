<template>
  <div class="payment-page">
    <div class="page-header">
      <h2>充值通知日志</h2><el-button @click="loadList">
        刷新
      </el-button>
...
<script setup lang="ts">import { onMounted, reactive, ref } from 'vue';import { rechargeService, type PayNotifyLog } from '@/services';const loading=ref(false);const list=ref<PayNotifyLog[]>([]);const detailVisible=ref(false);const detailData=ref<Record<string,any>>({});const query=reactive({tenantId:0,orderNo:'',notifyStatus:0});const loadList=async()=>{loading.value=true;try{const res=await rechargeService.getRechargeNotifyLogList({...query,tenantId:query.tenantId||undefined,orderNo:query.orderNo||undefined,notifyStatus:query.notifyStatus||undefined,limit:100});list.value=res.data||[]}finally{loading.value=false}};const showDetail=async(row:PayNotifyLog)=>{const res=await rechargeService.getRechargeNotifyLogDetail(row.id,row.tenantId);detailData.value=res.data||row;detailVisible.value=true};onMounted(loadList)</script>
<style scoped></style>
